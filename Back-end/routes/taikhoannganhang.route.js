...
router.get("/:soTK", async (req, res) => {
  ...
  try {
    const rows = await taikhoannganhangModel.getInfoNotMeBySoTK(
      userId,
      req.params.soTK
    );
    if (rows.length === 0) {
      ...
      if (rows[0].id !== userId) res.json(rows[0]);
      else
        res.status(409).json({
...
router.post("/", async (req, res) => {
  ...
  const tknh = await taikhoannganhangModel.patch(results.insertId, {
