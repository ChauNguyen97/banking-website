...
router.post("/noibo", verifyAccessToken, async (req, res, next) => {
  ...
  var ngayCK = new Date();
  try {
    const rows = await chuyenKhoanModel.chuyenKhoanNoiBo(
      userId,
      req.body,
      ngayCK
    );
  ...
  try {
    ...
    if (checksum == hashDigest.toString(CryptoJS.enc.Hex)) {
      ...
      if (valid) {
        ...
        //chuyen khoan
        const rows = await chuyenKhoanModel.NopTienVaoTaiKhoan(
          entity,
          new Date()
        );
...
router.post("/trutien", verifyAccessToken, async (req, res, next) => {
  ...
  try {
    ...
    var data;
    if (
      rowsDoiTac[0].chuanChuKy.toUpperCase() == otps.SIGNATURE.PGP.toUpperCase()
    ) {
    ...
    if (resAPI.status === 200) {
      ...
      const rows = await chuyenKhoanModel.chuyenKhoanLienNH(
        userId,
        entity,
        new Date()
      );
